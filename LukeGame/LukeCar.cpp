...
const FName ALukeCar::LookUpBinding("LookUp");
const FName ALukeCar::LookRightBinding("LookRight");
const FName ALukeCar::EngineAudioRPM("RPM");
...
ALukeCar::ALukeCar()
{
	...
	Vehicle4W->WheelSetups[2].WheelClass = UMyVehicleWheel::StaticClass();
	Vehicle4W->WheelSetups[2].BoneName = FName("PhysWheel_BL");
	Vehicle4W->WheelSetups[2].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[3].WheelClass = UMyVehicleWheel::StaticClass();
...
void ALukeCar::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward", this, &ALukeCar::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &ALukeCar::MoveRight);
	PlayerInputComponent->BindAxis(LookUpBinding);
	PlayerInputComponent->BindAxis(LookRightBinding);

	PlayerInputComponent->BindAction("EnterVehicle", IE_Pressed, this, &ALukeCar::Exit);

	PlayerInputComponent->BindAction("Handbrake", IE_Pressed, this, &ALukeCar::OnHandbrakePressed);
	PlayerInputComponent->BindAction("Handbrake", IE_Released, this, &ALukeCar::OnHandbrakeReleased);
	PlayerInputComponent->BindAction("SwitchCamera", IE_Pressed, this, &ALukeCar::OnToggleCamera);

	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &ALukeCar::OnResetVR);
}

void ALukeCar::MoveForward(float Val)
{
	GetVehicleMovementComponent()->SetThrottleInput(Val);

}

void ALukeCar::EnterCharacter(ALukeGameCharacter* Character)
{
	if (Character)
	{
		...
		//Character->AttachToComponent(GetMesh(), TransformRules);
...
void ALukeCar::Exit()
{
	...
	if (OwningCharacter)
	{
		//OwningCharacter->DetachRootComponentFromParent();
		//OwningCharacter->AddActorLocalOffset(FVector(0.0f, 400.0f, 100.0f));
		//OwningCharacter->SetActorEnableCollision(true);
		//OwningCharacter->SetActorRotation(FRotator::ZeroRotator);

		if (Controller)
		{
			Controller->Possess(OwningCharacter);
			OwningCharacter = NULL;
		}
	}
}

bool ALukeCar::ServerExit_Validate()
{
	return true;
}

void ALukeCar::ServerExit_Implementation()
{
	Exit();
}

void ALukeCar::MoveRight(float Val)
{
	GetVehicleMovementComponent()->SetSteeringInput(Val);
}

void ALukeCar::OnHandbrakePressed()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void ALukeCar::OnHandbrakeReleased()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void ALukeCar::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void ALukeCar::EnableIncarView(const bool bState)
{
...
void ALukeCar::Tick(float Delta)
{
	Super::Tick(Delta);
...
void ALukeCar::UpdateHUDStrings()
{
...
void ALukeCar::UpdatePhysicsMaterial()
{
